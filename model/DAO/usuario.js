/******************************************************************************
 * Objetivo: Arquivo responsavel pelo CRUD de dados no MySQL referente aos usuários
 * Data: 24/11/2025
...
const getSelectAllUsers = async () => {
  try {
    //script SQL
    let sql = `select * from tb_usuarios where id_status = 1 order by id desc`;
...
/* Vazio, undefined e null estão sendo cadastrados no banco. Tratar pelo controller ou front */
const setInsertUser = async (usuario) => {
  try {
    let sql = `INSERT INTO tb_usuarios (
                        nome,
                        genero,
						            email,
                        senha,
